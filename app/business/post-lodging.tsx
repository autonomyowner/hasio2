...
export default function PostLodgingScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      // Upload images to R2
      const uploadedImages = await uploadMultipleToR2(images, "lodging");

      // Create lodging in Convex
      await createLodging({
        name: name.trim(),
        nameAr: nameAr.trim(),
        type,
        ...
        neighborhoodAr: neighborhoodAr.trim() || undefined,
        priceRange: priceRange.trim() || undefined,
        images: uploadedImages,
        amenities: amenities.split(",").map((a) => a.trim()).filter(Boolean),
        amenitiesAr: amenitiesAr.split(",").map((a) => a.trim()).filter(Boolean),
